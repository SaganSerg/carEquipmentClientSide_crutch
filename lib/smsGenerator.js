        ...
        db.run('INSERT INTO smscodes (smscode_value, smscode_telephoneSerialNumber, telephone_id) VALUE (?, ?, ?)', [smsCode, telephoneSerialNumber, telephoneId], (err, insertSmscodesRows) => {
            ...
            if (err?.code === "ER_DUP_ENTRY") {
                return res.status(200).json({
                    "result": 'ERR',
                    "discription": `For this telephone number the SMS request has already been made -- ${addTextComment('ошибка БД where ER_DUP_ENTRY INSERT INTO smscodes (smscode_value, smscode_telephoneSerialNumber, telephone_id) VALUE (?, ?, ?)')}`,
                    "responseCode": "0011001",
                })
            }
            if (err) return res.status(200).json({
                "result": 'ERR',
                "discription": `Something went wrong${addTextComment('ошибка БД INSERT INTO smscodes (smscode_value, smscode_telephoneSerialNumber, telephone_id) VALUE (?, ?, ?)')}`,
