...
let makeConnectionUpdateUsersReturnJSONtest = (arr) => {
    ...
    */
    ...
    let userName = userNameFull
    let objOwnerId = objOwnerId1 // не важно
    counter = [] // это мы обнуляем список вызовов, чтобы потом иметь возможность снова
    let result = makeConnectionUpdateUsersReturnJSON(req, res, next, db, getResponseJSON, addTextInComment, getTokenFunction, userId, credentials, tokenExpire, longTokenExpire, userName, objOwnerId)
    responseCode = `0001003`
    let check = getResponseJSON(responseCode, 'ощибка БД UPDATE users SET user_name = ? WHERE user_id = ?')
